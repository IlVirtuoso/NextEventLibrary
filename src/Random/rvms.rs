...
/* ============================================ 
  ...
  */ {
    if (x < n) {
        1.0 - InBeta((x + 1) as f64, (n - x) as f64, p)
    } else {
        (1.0)
    }
}
...
/* ===================================== 
  ...
  * =====================================
  */ {
    return (1.0 - ((x + 1) as f64 * (p).ln()).exp());
}
...
/* =========================================== 
  ...
  */ {
    ...
    s = LogChoose((n + x - 1) as f64, x as f64);
    t = x as f64 * (p).ln() + n as f64 * (1.0 - p).ln();
    return ((s + t).exp());
}
...
/* =========================================== 
  ...
  * ===========================================
  */ {
    return (1.0 - InBeta((x + 1) as f64, n as f64, p));
}
...
/* ================================================== 
  ...
  */ {
    ...
    if cdfPascal(n, p, x as u32) <= u {
        while (cdfPascal(n, p, x as u32) <= u) {
            x += 1.0;
        }
    } else if (cdfPascal(n, p, 0) > u) {
        while (cdfPascal(n, p, x as u32 - 1) > u) {
            x -= 1.0;
        }
    } else {
        x = 0.0;
...
/* ===================================
  ...
  */ {
    ...
    t = -m + x as f64 * (m).ln() - LogFactorial(x);
    return ((t).exp());
}
...
/* =================================== 
  ...
  * ===================================
  */ {
    return (1.0 - InGamma((x + 1) as f64, m));
}
...
/* ============================================ 
  ...
  */ {
    ...
    t = (n - 1) as f64 * (x / b).ln() - (x / b) - (b).ln() - LogGamma(n as f64);
    return ((t).exp());
}
...
/* ===================================== 
  ...
  * =====================================
  */ {
    let (mut t, s) = (0.0, n as f64 / 2.0);

    t = (s - 1.0) * (x / 2.0).ln() - (x / 2.0) - (2.0f64).ln() - LogGamma(s);
    return ((t).exp());
}
...
/* ===================================== 
  ...
  * =====================================
  */ {
    return (InGamma(n as f64 / 2.0, x / 2.0));
}
...
/* ===================================== 
  ...
  * =====================================
  */ {
    let (mut t, mut x) = (0.0, n as f64); /* initialize to the mean, then */
...
/* =================================== 
  ...
  */ {
    let (mut s, mut t) = (0.0, 0.0);
    s = -0.5 * (n + 1) as f64 * (1.0 + ((x * x) / n as f64).ln());
    t = -LogBeta(0.5, n as f64 / 2.0);
...
/* =================================== 
  ...
  */ {
    ...
    loop {
        /* use Newton-Raphson iteration */
        t = x;
        x = t + (u - cdfStudent(n as f64, t)) / pdfStudent(n, t);
        if !((x - t).abs() >= TINY) {
            break;
        }
    }
...
/* ======================================================================== 
  ...
  */ {
    ...
    temp = (a - 0.5) * (a + 4.5).ln() - (a + 4.5) + (*SQRT2PI * sum).ln();
    return (temp);
}
...
/* ========================================================================
  ...
  */ {
    ...
    let mut n: u32 = 0;
    if (x > 0.0) {
        factor = (-x + a * (x).ln() - LogGamma(a)).exp();
    } else {
        factor = 0.0;
    }
    if (x < a + 1.0) {
...
mod tests {
    ...
    fn test_distributions() {
        assert!(idfStudent(10, 0.8)- 0.879058 <= TINY);
        assert!(idfStudent(10, 0.975)- 2.228139 <= TINY);
        assert!(idfStudent(100, 0.975)- 1.983972 <= TINY);
        assert!(idfChisquare(10, 0.5)- 9.341818 <= TINY);
        assert!(idfChisquare(15, 0.8)- 19.310657117 <= TINY);
        assert!(idfErlang(16, 4.0, 0.878)- 82.934761 <= TINY);
        assert!(idfErlang(20, 7.0, 0.113)- 103.476309052 <= TINY);
        assert!(idfPoisson(16.0, 0.878)- 21 ==0);
        assert!(idfPoisson(19.0, 0.231)- 16 == 0);
        assert!(idfNormal(9.0, 2.0, 0.66)- 9.824926258 <= TINY);
        assert!(idfNormal(-19.0, 30.4, 0.81) -16.015153 <= TINY);
        assert!(idfPascal(23, 0.11, 0.90)- 5 == 0);
        assert!(idfPascal(6, 0.5, 0.5)- 6 == 0);
        assert!(idfBinomial(23, 0.11, 0.90)- 5 == 0);
        assert!(idfBinomial(6, 0.5, 0.5)- 3 == 0);
    }
}
